...
//<2014/05/25-tedwu, Battery OTP modification.
//<2014/04/02-Yuting Shih. Add the temperature level of battery check.
#if defined( ARIMA_LO1_HW ) || defined( ARIMA_LO2_HW )
unsigned int g_BatTempLevel = TEMP_NEG_10_TO_POS_45;
#endif
extern INT32 g_new_temperature;
//>2014/04/02-Yuting Shih.
//<2014/06/12-tedwu, Customize time and UI_SOC for SYNC_TO_REAL.
extern U32 g_spm_timer;
extern bool bat_spm_timeout_sync_ui_soc;
//>2014/06/12-tedwu
#if defined(ARIMA_LO1_HW) || defined(ARIMA_LO2_HW)
static kal_bool g_charging_full_first_flag = KAL_FALSE;  //<2014/06/16-tedwu, fix the not charged full.//>
#endif
//>2014/05/25-tedwu
...
#else	
    kal_uint32 tmp32;
//<2014/10/20-45425-Richard Li,[5503][Battery][Common] Remove the previous delay solution for CIS charger issue and apply the reset charger IC manually solution
//<2014/09/29-44550-Richard Li,[5503][Common][Battery] Apply the delay solution for both Lo1 and Lo2 to avoid OVP issue caused by CIS charger
//<2014/07/04-tedwu, (mtk patch)add delay for LO2 charger.
    ...
    {
        if(first_in == KAL_TRUE)
        {
            msleep(1000);
            first_in = KAL_FALSE;
            battery_xlog_printk(BAT_LOG_FULL, "add delay for LO2 charger\n");
        }
    }
    #endif
#endif
//>2014/07/04-tedwu
//<2014/09/29-44550-Richard Li
//<2014/10/20-45425-Richard Li
	...
	static kal_uint32 timer_counter = (ONEHUNDRED_PERCENT_TRACKING_TIME/BAT_TASK_PERIOD);
	 ...
	 else	
	 {
		...
		if(BMT_status.UI_SOC>=99)
        {
			...
			resetBatteryMeter = KAL_FALSE;
            #if defined(ARIMA_LO1_HW) || defined(ARIMA_LO2_HW)
            g_charging_full_first_flag = KAL_TRUE;  //<2014/06/16-tedwu, fix the not charged full.//>
            #endif
...
//<2014/06/11-tedwu, Customize time and UI_SOC for SYNC_TO_REAL.
		   kal_bool time_out = KAL_FALSE;
    ...
    {   
        ...
        {   
            timer_counter += ((g_spm_timer + (g_spm_timer/3))/BAT_TASK_PERIOD);
            bat_spm_timeout_sync_ui_soc = false;
        }
        else
            timer_counter ++;

		battery_xlog_printk(BAT_LOG_CRTI, "=~= timer_counter = %d\r\n", timer_counter);

        if (BMT_status.UI_SOC > SYNC_TO_REAL_UI_SOC_FIRST)
        {
            if(timer_counter >= (SYNC_TO_REAL_TRACKING_TIME_FIRST/BAT_TASK_PERIOD))
                time_out = KAL_TRUE;
        }
        else if (BMT_status.UI_SOC > SYNC_TO_REAL_UI_SOC_SECOND)
        {
            if(timer_counter >= (SYNC_TO_REAL_TRACKING_TIME_SECOND/BAT_TASK_PERIOD))
                time_out = KAL_TRUE;
        }
        else if (BMT_status.UI_SOC > SYNC_TO_REAL_UI_SOC_THIRD)
        {
            if(timer_counter >= (SYNC_TO_REAL_TRACKING_TIME_THIRD/BAT_TASK_PERIOD))
                time_out = KAL_TRUE;
        }
        else
        {
...
//>2014/06/11-tedwu
	...
	{
//<2014/06/16-tedwu, fix the not charged full.
		timer_counter = 0;
        #if defined(ARIMA_LO1_HW) || defined(ARIMA_LO2_HW)
        if (g_charging_full_first_flag)
            BMT_status.UI_SOC = 99;
        else
            BMT_status.UI_SOC = BMT_status.SOC;

        battery_xlog_printk(BAT_LOG_CRTI, "UI_SOC=(%d), SOC=(%d), g_charging_full_first_flag=(%d)\r\n", BMT_status.UI_SOC, BMT_status.SOC, g_charging_full_first_flag);
        #else
        BMT_status.UI_SOC = BMT_status.SOC;
        #endif
//>2014/06/16-tedwu
...
//<2014/05/25-tedwu, Battery OTP modification.
#if 0
//<2014/04/08-Yuting Shih. Add the temperature level of battery check.
  #if defined( ARIMA_LO1_HW ) || defined( ARIMA_LO2_HW )
  ...
  #endif /* End.. (ARIMA_LO1_HW)||.. */
//>2014/04/08-Yuting Shih.
#endif
//>2014/05/25-tedwu
    ...
    #if defined(ARIMA_LO1_HW) || defined(ARIMA_LO2_HW)
    g_charging_full_first_flag = KAL_FALSE;  //<2014/06/16-tedwu, fix the not charged full.//>
    #endif
...
//<2014/04/07-Yuting Shih. Add the temperature level of battery check.
  ...
  if( BMT_status.temperature < MIN_CHARGE_TEMPERATURE )
  {
    ...
    //g_BatTempLevel      = TEMP_BELOW_NEG_10; //Ted masked, already done in mt_battery_notify_VBatTemp_check()
    g_batt_temp_status  = TEMP_POS_LOW;
  ...
  {
    if( BMT_status.temperature > MIN_RECHARGE_TEMPERATURE )
    {
      battery_xlog_printk( BAT_LOG_CRTI, "[BATTERY] Battery Temperature raise from %d to %d(%d), allow charging!!\n\r", 
            MIN_CHARGE_TEMPERATURE, BMT_status.temperature, MIN_RECHARGE_TEMPERATURE );

      //g_BatTempLevel      = TEMP_NEG_5_TO_POS_0; //Ted masked, already done in mt_battery_notify_VBatTemp_check()
      g_batt_temp_status  = TEMP_POS_NORMAL;
      BMT_status.bat_charging_state = CHR_PRE;
    }
    else
	...
	{
//<2014/04/10-Yuting Shih. Add the temperature level of battery check.
#if 0  //Ted masked, already done in mt_battery_notify_VBatTemp_check()
#if defined( ARIMA_LO1_HW ) || defined( ARIMA_LO2_HW )
		g_BatteryNotifyCode |= 0x0020;
		g_BatTempLevel      = TEMP_ABOVE_POS_60;
#endif
#endif
//>2014/04/10-Yuting Shih.
      #if 0  //<2014/05/18-tedwu //>
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Battery Over Temperature !!\n\r"); 
		g_batt_temp_status =TEMP_POS_HIGH;
		return PMU_STATUS_FAIL; 
      #else
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Battery Over MAX_CHARGE_TEMPERATURE !\n\r"); 
		g_batt_temp_status =TEMP_POS_HIGH;
        //BMT_status.bat_charging_state = CHR_HOLD;
		return PMU_STATUS_FAIL; 
      #endif
	...
	{
//<2014/04/03-Yuting Shih. Add the temperature level of battery check.
		//g_BatteryNotifyCode &= ~(0x0020);  //Ted masked, already done in mt_battery_notify_VBatTemp_check()
  #if defined( ARIMA_LO1_HW ) || defined( ARIMA_LO2_HW )
		if( BMT_status.temperature < MAX_RECHARGE_TEMPERATURE ) {
			battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Battery Temperature down from %d to %d(%d), allow charging!!\n\r", 
				MAX_CHARGE_TEMPERATURE, BMT_status.temperature, MAX_RECHARGE_TEMPERATURE );
			//BatTempLevel      = TEMP_POS_10_TO_POS_45;
			g_batt_temp_status = TEMP_POS_NORMAL;
			BMT_status.bat_charging_state = CHR_PRE;
			//return PMU_STATUS_OK;
		}
  ...
  #endif
//>2014/04/03-Yuting Shih.
		else {
		    //BMT_status.bat_charging_state = CHR_HOLD;  //<2014/05/18-tedwu //>
			return PMU_STATUS_FAIL;
		}
	}
	else
...
//<2014/04/07-Yuting Shih. Add the temperature level of battery check.
  #if defined( ARIMA_LO1_HW ) || defined( ARIMA_LO2_HW )
    /*  //<2014/05/18-tedwu //>
    if( BMT_status.temperature > TEMP_POS_60_THRES_MINUS_X_DEGREE )
    {
      g_BatteryNotifyCode |= 0x0040;
      g_BatTempLevel      = TEMP_POS_55_TO_POS_57;
      g_batt_temp_status  = TEMP_POS_PAUSE;
      BMT_status.bat_charging_state = CHR_HOLD;
    }
    else if( g_batt_temp_status == TEMP_POS_PAUSE )
    {
      g_BatteryNotifyCode &= ~(0x0040);
      if( BMT_status.temperature <= MAX_RECHARGE_TEMPERATURE )
      {
        g_BatTempLevel      = TEMP_POS_10_TO_POS_45;
        g_batt_temp_status  = TEMP_POS_NORMAL;
        BMT_status.bat_charging_state = CHR_PRE;
      }
      else
      {
        g_BatTempLevel      = TEMP_POS_45_TO_POS_55;
        BMT_status.bat_charging_state = CHR_HOLD;
      }
    }
    else
    */
    {
#if 1
      if( BMT_status.temperature > TEMP_POS_45_THRESHOLD )
#else
      if(( BMT_status.temperature > TEMP_POS_45_THRESHOLD ) &&
         ( BMT_status.temperature <= MAX_CHARGE_TEMPERATURE /* TEMP_POS_60_THRES_MINUS_X_DEGREE */ ))
#endif
      {
      /* 
          <= 4.0V, 400mA
      ...
      */
        //g_BatTempLevel      = TEMP_POS_45_TO_POS_55;
        if( BMT_status.bat_vol > TEMP_CHECK_CHARGING_VOLTAGE )
        {
          //BMT_status.bat_charging_state = CHR_HOLD;
          g_batt_temp_status =TEMP_POS_HIGH;  //<2014/05/25-tedwu, added for Battery OTP modification. //>
          return PMU_STATUS_FAIL;
        }
        else
        {
          g_batt_temp_status  = TEMP_POS_DEC_CHARGE;
          //BMT_status.bat_charging_state = CHR_CC;
        }
      }
      else if( g_batt_temp_status == TEMP_POS_DEC_CHARGE )
      {
        if( BMT_status.temperature < MAX_RECHARGE_TEMPERATURE )
        {
		  g_batt_temp_status = TEMP_POS_NORMAL;
        ...
        else if( BMT_status.bat_vol > TEMP_CHECK_CHARGING_VOLTAGE )
        {
          //BMT_status.bat_charging_state = CHR_HOLD;
          g_batt_temp_status =TEMP_POS_HIGH;  //<2014/05/25-tedwu, added for Battery OTP modification. //>
          return PMU_STATUS_FAIL;
      ...
      {
      //g_BatTempLevel = TEMP_NEG_10_TO_POS_45;
        //g_BatteryNotifyCode &= ~(0x0002|0x0020|0x0040);  //Ted masked.
        //BMT_status.bat_charging_state = CHR_PRE;  //Ted masked.
        g_batt_temp_status = TEMP_POS_NORMAL;
      }
    }
  #endif
//>2014/04/07-Yuting Shih.
	}
	return PMU_STATUS_OK;
}
#endif
...
//<2014/06/06-tedwu, For battery temperature protection.
    g_new_temperature = temperature;
	BMT_status.old_temperature = BMT_status.temperature;
//>2014/06/06-tedwu
...
static void mt_battery_notify_VBatTemp_check(void)
{
	...
	if(BMT_status.temperature > TEMP_POS_60_THRESHOLD)  //<2014/05/18-tedwu //>
    {
//<2014/04/10-Yuting Shih. Add the temperature level of battery check.
#if defined( ARIMA_LO1_HW ) || defined( ARIMA_LO2_HW )
        g_BatTempLevel = TEMP_ABOVE_POS_60;
...
//<2014/04/10-Yuting Shih. Add the temperature level of battery check.
#if defined( ARIMA_LO1_HW ) || defined( ARIMA_LO2_HW )
    else if( BMT_status.temperature > TEMP_POS_60_THRES_NEAR_X_DEGREE )
    {
      g_BatteryNotifyCode |= 0x0002;
      g_BatTempLevel = TEMP_POS_57_TO_POS_60;
    }
#endif
//>2014/04/10-Yuting Shih.
#if defined(MTK_JEITA_STANDARD_SUPPORT)
...
#endif
//<2014/04/01-Yuting Shih. Add the temperature level of battery check.
#if defined( ARIMA_LO1_HW ) || defined( ARIMA_LO2_HW )
  else
  {
    //g_BatteryNotifyCode &= ~(0x0002);
    //g_BatteryNotifyCode &= ~(0x0020);
    battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] battery temperature: %d\n", BMT_status.temperature );
    if( BMT_status.temperature > TEMP_POS_60_THRES_MINUS_X_DEGREE )
    {
      if (KAL_TRUE == BMT_status.charger_exist)  //<2014/05/17-tedwu, Battery OTP modification. //>
        g_BatteryNotifyCode |= 0x0040;
      g_BatTempLevel      = TEMP_POS_55_TO_POS_57;
      //g_batt_temp_status  = TEMP_POS_PAUSE;  //<2014/05/25-tedwu, Battery OTP modification. //>
    }
...
/*  //<2014/05/25-tedwu, Battery OTP modification. //>
    ...
    {
      g_BatTempLevel = TEMP_POS_0_TO_POS_10;
    }
    else if( BMT_status.temperature >= TEMP_NEG_10_THRES_PLUS_X_DEGREE )
    {
      g_BatTempLevel = TEMP_NEG_5_TO_POS_0;
    }
    else if( BMT_status.temperature >= TEMP_NEG_10_THRESHOLD )
    {
      g_BatTempLevel = TEMP_NEG_10_TO_NEG_5;
    }
*/
    else if( BMT_status.temperature >= TEMP_NEG_10_THRESHOLD )
    {
    ...
    //g_BatteryNotifyCode |= 0x0100;
      g_BatTempLevel = TEMP_BELOW_NEG_10;
    }
  #endif
  }
#endif /* End.. ARIMA_LO1_HW()||.. */
//>2014/04/01-Yuting Shih.
#endif
...
static void mt_battery_thermal_check(void)
{
	if( (g_battery_thermal_throttling_flag==1) || (g_battery_thermal_throttling_flag==3) )
    {
        ...
        //ignore default rule
#else    
		if(BMT_status.temperature > TEMP_POS_60_THRESHOLD)  //<2014/05/17-tedwu, Battery OTP modification. //>
        {
            #if defined(CONFIG_POWER_EXT)
            ...
            #else
            {
                ...
                {
                    //<2014/06/05-tedwu, Battery OTP modification.
                    //Do not shutdown directly, need show message first.
                    #if defined(ARIMA_LO1_HW) || defined(ARIMA_LO2_HW)
                    battery_xlog_printk(BAT_LOG_CRTI, "Tbat(%d)>60, bypass thermal_check.\n", BMT_status.temperature);
                    #else
...
//<2014/04/10-Yuting Shih. Add the temperature level of battery check.
      ...
      else
      {
        ...
        {
          battery_xlog_printk(BAT_LOG_FULL, "[BATTERY] boot mode = %d, temperature non-check\n", g_platform_boot_mode );
        }
        else
        {
          if(( BMT_status.charger_exist == KAL_TRUE ) && ( BMT_status.bat_charging_state == CHR_HOLD ))
          {
          struct battery_data *bat_data = &battery_main;
          struct power_supply *bat_psy = &bat_data->psy;
